...
export const buildPersona = (analysedResume) => {
  // WEIRD behavior. OpenAI always returns text. Sometimes, there is an extra `\` before `\n`. 
  // So we need to identify the dividing token.

  let splittingToken
  if(analysedResume.match("\\\\n")) {
    splittingToken = "\\n"
  } else {
    splittingToken = "\n"
  }

  const parts = analysedResume.split(splittingToken)
...
const hasOnlyKey = (partsSplitByColon) => {
  if(partsSplitByColon.length == 2) {
    if(partsSplitByColon[1] === " " || partsSplitByColon[1] === "") {
      return true
    }
  }

  if(partsSplitByColon.length == 1) {
    return true
  }
